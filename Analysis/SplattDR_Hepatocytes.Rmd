...
simDR<-readRDS("~/Documents/Documents/Summer_2019/Mouse_Liver/Rance/snSeq-DGE/SplattDR/simDR.RData")
simulated.data<-logcounts(simDR)
simulated.cell.metadata<-colData(simDR)
simulated.gene.metadata<-rowData(simDR)
...
source('~/Documents/Documents/Summer_2019/Mouse_Liver/Rance/snSeq-DGE/R/BayesFactorMultiple.R')
  ...
  for(j in 1:nrow(data[["0"]]))
  {
    bf_multiple_01[[j]]<-Bayes_factor_multiple(
                           ...
                           prior_alt =prior_Alter[2],
                           prior_null = prior_Null[2])
  }
```
...
```{r}
#Check the 161 genes that are designated as "Positive" by the second level of the BF test
genes_DE_second_BF_01<-rownames(data_non_sig_genes_BF[["0"]])[which(Decision_bf_pairwise_non_sig_genes_BF_01_new == "Positive" & True_decision_non_sig_genes_BF_01 == "Positive")]
simulated.gene.metadata[genes_DE_second_BF_01,]
data_group_summary(simulated.data_dose[,c(genes_DE_second_BF_01[2],"Dose")])
```
...
value1<-c(32,8,536,678,49,5,519,681,80,13,488,673,118,17,450,669,
          199,73,369,613,327,301,241,385,504,592,64,94,566,686,2,0)
...
ggplot(cf, aes(fill=type, y=value1, x=as.factor(dose))) + 
  geom_text(aes(label=value1), position = position_dodge(width=1),
            ...
            xlab("Dose")+ ylab("Confusion Matrix Values")+
             ggtitle("Pairwise testing for DE genes")+ 
...
value1<-c(568,686,558,9548,945,5115,181,5119,965,5320,161,4914,958,6036,168,4198,967,6270,
          159,3964,925,4936,201,5298,729,2607,397,7627)
#value2<-c(443,405,683,9829,443,407,683,9827,443,407,683,9827,456,407,670,9827,72,439,1054,9795,
         # 443,407,683,9827)
cf <- data.frame(method,type,value1)#,value2)
ggplot(cf, aes(fill=type, y=value1, x=as.factor(method))) + 
  geom_text(aes(label=value1), position = position_dodge(width=1),
            ...
            xlab("Method")+ ylab("Confusion Matrix Values")+
             ggtitle("Simulated Data Performance without log-fold changes")+ 
              theme(plot.title = element_text(color="red", size=10, face="bold.italic"))
```
```{r}
method <- c(rep("BF" , 4) , rep("Anova" , 4) , rep("KW" , 4),rep("MAST" , 4),rep("Wilcox" , 4) )
type <- rep(c("TP" , "FP" , "FN","TN") , 5)
value<-c(359,837,241,1063,380,870,220,1630,380,870,220,1630,
         506,1812,94,88,503,1681,97,219)
