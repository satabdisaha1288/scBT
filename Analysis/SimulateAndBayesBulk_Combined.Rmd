...
i = 0
for (ct in unique(splat.long$cell)[1:2]){
  for (de.prob in c(0.1, 0.25, 0.5, 0.75)){
    for (true.count in 1:2){
      for (iter in 1:10){
        ...
        if (true.count == 2){
          ...
          dose.prob = rep(param.medians$nCells, length(dose.names))/(sum(rep(param.medians$nCells, length(dose.names))))
        }
        ...
        #Set and record seed
        sim.settings.list[[simName]]$seed = sample(1:1000, 1)
...
```{r}
#de.probability = c(0.1, 0.25, 0.5, 0.75)
de.probability = c(0.25, 0.75)
stat.out = list()
simDR = list()
```
...
```{r}
for (simNo in names(sim.settings.list)[1:4]) {
  vec_elem = simNo
  message(vec_elem)
