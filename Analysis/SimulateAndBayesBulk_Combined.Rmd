...
for (test in names(stat.out)){
  gene.meta = rowData(simDR.list[[test]][rownames(stat.out[[test]]$bf_1),])
  ...
  bf = caret::confusionMatrix(df$real, df$bf)
  ...
  anova = caret::confusionMatrix(df$real, df$anova)
  confusion.df = rbind(confusion.df, data.frame(dataset = substring(test, 1, 60),
                                                test = rep('ANOVA', 4),
                                                type = c('TN', 'FN','FP', 'TP'),
                                                anova$table
  ...
  kw = caret::confusionMatrix(df$real, df$kw)
  confusion.df = rbind(confusion.df, data.frame(dataset = substring(test, 1, 60),
                                                test = rep('KW', 4),
                                                type = c('TN', 'FN','FP', 'TP'),
                                                kw$table
  ...
  wrs = caret::confusionMatrix(df$real, df$wrs)
  confusion.df = rbind(confusion.df, data.frame(dataset = substring(test, 1, 60),
                                                test = rep('wrs', 4),
                                                type = c('TN', 'FN','FP', 'TP'),
                                                wrs$table
...
runLRT = function(data.list){
  for (dose in names(data.list)){
    data.list[[dose]] = t(as.matrix(data.list[[dose]]))
  }
  ...
  for(g in 1: nrow(data.list[["0"]])){
    print(g)
    LRT_mult[[g]]<-LRT_multiple_groups(data = list(data.list[["0"]][g,][data_ind[["0"]][g,] > 0],
                        ...
                        data_ind = list(data_ind[["0"]][g,],
                                    ...
                                    data_ind[["10"]][g,],
                                    data_ind[["30"]][g,]))
    LRT_mult[[g]]$Gene = rownames(data.list[["0"]][g,])
  }
}
priors.out = calc_priors(simulated.data, simulated.cell.metadata, simulated.gene.metadata)
o = runLRT(priors.out$split.simulated)
```
