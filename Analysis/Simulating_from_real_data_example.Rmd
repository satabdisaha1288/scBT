...
allReal = readRDS('C.RData')
realData = allReal %>% filter(category == 'pass') %>% filter(Cells == 7)
```

### Estimate parameters
Parameters will be estimated for each dose group independently and then combined into a single data frame.
_This can be slow depending on the number of cores used_
```{r}
realData = rbind(
...
realData = realData %>% filter(category == 'pass') #Genes that are all zero's or could not be modeled are given the "fail" flag
```
