...
Generate the data (p genes and n cells) according to the Bayesian model scheme

```{r}
#K is the number of groups, n=number of cells;
#p=number of genes
#sigma_j_squared ~ Inverse Gamma( a_sigma, b_sigma)
#omega_k_j ~ Beta( a_k_w,b_k_w)
#mu_k_j ~Normal(m_k_0, tau_k_mu*sigma_j_squared)
#R_i_j ~ Bernoulli(omega_k_j)
#Y_i_j|R_i_j = 1 ~ Normal(u_k_j,sigma_j_squared)# this is the hurdle normal data
#All the hyperparameters have been learnt from real data
...
sigma_squared<-invgamma::rinvgamma(p,shape=a_sigma,rate=b_sigma)
...
{
  colnames(mydata)[i]<-sprintf("%s%i", "gene",i)
}
colnames(mydata[ncol(mydata)])<-"Dose"
