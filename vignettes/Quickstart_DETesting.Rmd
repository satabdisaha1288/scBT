...
- priors
```{r}
priors$priors #Subset once we have the data ouput as final
```
...
```{r}
#R Run
bayes.out = new_bayesDETest(priors)
```
...
How to pick the treshold (1/10 or 1/3) cite the paper Kass Bayes Factor
```{r}
hist(bayes.out$exp_bf) #Revisit for a useful plot. Add lines at 1/10 or 1/3
#Barplot of how many genes lie within boundaries/ cummulative plot/histogram with line at defined values
```
